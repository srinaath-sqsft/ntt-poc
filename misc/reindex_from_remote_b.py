...
def read():
    ...
    for doc in scan(es, timeout=None, request_timeout=100,
                    ...
                    scroll='1m',
                    raise_on_error=True, size=500, query={
                "query": {
                    "bool": {
                        "must": [
                            {
                                "query_string": {
                                    "query": "agent.name: rum-js AND user_agent.os.name: Android AND user_agent.device.name: LG-$2"
                                }
                            },
                            ...
                            {"exists": {"field": "trace.id"}},
                            {
                                "script": {
                                    "script": {
                                        "source": "doc['trace.id'].value.endsWith('e')", "lang": "painless"
                                    }
                                }
                            }
                        ]
