...
const onSubmit = async (values) => {
  // Helps with renaming the place order transaction
  const placeOrderTr = apm.startTransaction("/place-order", "user-interaction", {
    canReuse: true,
    managed: true,
  });
  placeOrderTr.addLabels({ "userId": Cookies.get("local_user_id") })
  ...
  await placeOrderTr.end();
  window.sessionStorage.setItem("order", JSON.stringify(data));
  console.log("queue 1: " + window.elasticApm.serviceFactory.getService("ApmServer").queue.items.length)
  window.elasticApm.serviceFactory.getService("ApmServer").queue.flush();

  setTimeout(function() {
    window.location.replace("/cart/checkout");
    }, 1000)

  console.log("queue 2: " + window.elasticApm.serviceFactory.getService("ApmServer").queue.items.length)
};
